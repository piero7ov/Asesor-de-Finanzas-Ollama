...
/* ==========================================================
   ...
   - Salida simple en <pre> (SIN Markdown aún)
   - Modelo/URL fijos (no visibles)
   - Timeout: 240s
   - max_execution_time: 300s
   - options: num_predict 400
   - Prompt con reglas: no dejar cálculos a medias + RESULTADO final dinámico
   ========================================================== */
...
/* Config fija (el usuario NO la ve) */
...
/* helper: normaliza números (acepta coma) */
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  // 2) Prompt con reglas
  $system =
    "Eres un consejero de finanzas personales BASICO para personas que se independizan.\n".
    "Responde en español, claro y práctico. NO recomiendes inversiones complejas.\n".
    "Usa los DATOS ESTRUCTURADOS si existen. Si faltan datos para calcular, NO inventes: haz hasta 3 preguntas concretas.\n".
    "\n".
    "REGLAS DE CALCULO:\n".
    "- Si haces una suma/resta/división, NO la dejes a medias: escribe siempre el resultado final.\n".
    "- Usa EUR (€) y números claros (un solo número, no rangos).\n".
    "\n".
    "FORMATO DE SALIDA:\n".
    "1) Explica en 3-6 líneas lo esencial.\n".
    "2) Si hubo cálculo, muestra una línea por cálculo con este formato: \"Calculo: <operacion> = <resultado>€\".\n".
    "3) Termina SIEMPRE con una última línea que empiece exactamente por \"RESULTADO:\" y que contenga el dato clave según el tipo de pregunta:\n".
    "   - Si pregunta \"cuánto me queda / ocio / margen\": usa \"Dinero_disponible_EUR=<numero>\".\n".
    "   - Si pregunta \"cuánto ahorrar para un objetivo\": usa \"Ahorro_mensual_EUR=<numero>\".\n".
    "   - Si pregunta \"financiar / pagar en X meses\": usa \"Cuota_mensual_EUR=<numero>\".\n".
    "   - Si no aplica cálculo: usa \"Conclusion=<frase corta>\".\n".
    "\n".
    "Ahora responde usando los datos y la pregunta.\n\n";
  ...
  <div class="card">
    <form method="post">
      <p class="hint">
        ...
        - “Quiero ahorrar 14.000€ en 24 meses, ¿cuánto debo apartar al mes?”<br>
        - “Quiero pagar un coche de 10.000€ en 36 meses, ¿cuánto es al mes?”
      </p>
      ...
      <div class="grid">
        <div>
          ...
          <input id="ingresos" name="ingresos" value="<?=h($ingresos)?>" placeholder="Ej: 1500">
        </div>
        <div>
          <label for="g_alquiler">Alquiler / Hipoteca (€)</label>
          <input id="g_alquiler" name="g_alquiler" value="<?=h($g_alquiler)?>" placeholder="Ej: 700">
        </div>
      </div>
      ...
      <div class="grid3" style="margin-top:12px;">
        <div>
          ...
          <input id="g_luz" name="g_luz" value="<?=h($g_luz)?>" placeholder="Ej: 40">
        </div>
        <div>
