...
function clean_ai_tips(string $txt): array {
  ...
  // Fallback si el modelo no obedeció: corta en frases
  if (count($tips) < 5) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($precio !== null && $plazoF !== null && $plazoF > 0) {
    $cuotaMensual = $precio / $plazoF; // básico sin intereses
  }

  // Diferencia viabilidad (si hay margen y ahorro mensual)
  $diff = null;
  if ($margen !== null && $ahorroMensual !== null) {
    $diff = $margen - $ahorroMensual; // si es negativo, te falta
  }

  // Tiempo para alcanzar meta (si hay margen > 0)
  $mesesExactos = null;
  $mesesRedondeo = null;
  if ($intentType === "tiempo" && $metaAmount !== null && $margen !== null && $margen > 0) {
    $mesesExactos = $metaAmount / $margen;
    $mesesRedondeo = (int)ceil($mesesExactos); // aproximado hacia arriba
  }
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    .cardHead{
      ...
      background:
        linear-gradient(180deg, rgba(14,165,233,.08), rgba(249,115,22,.05));
      display:flex;
      align-items:center;
      justify-content:space-between;
      gap: 12px;
    ...
    <div class="topbar-inner">
      <div class="brand">
        ...
        <div class="title">
          ...
          <p>PHP calcula y Ollama da tips. Sin Markdown por ahora</p>
        </div>
      </div>
    </div>
  </div>
