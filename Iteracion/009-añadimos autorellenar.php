...
/* ------------------------------
   ...
   - ask: calcula + IA + guarda historial
   - autofill: carga última consulta y solo rellena campos
-------------------------------- */
$action = $_POST["action"] ?? "ask";

if ($_SERVER["REQUEST_METHOD"] === "POST" && $action === "autofill") {
  $hist = history_load($histPath);

  if (!empty($hist) && is_array($hist[0])) {
    $last = $hist[0];

    $prompt = (string)($last["question"] ?? "");

    $form = $last["form"] ?? [];
    if (is_array($form)) {
      $ingresos     = (string)($form["ingresos"] ?? "");
      $g_alquiler   = (string)($form["g_alquiler"] ?? "");
      $g_luz        = (string)($form["g_luz"] ?? "");
      $g_agua       = (string)($form["g_agua"] ?? "");
      $g_internet   = (string)($form["g_internet"] ?? "");
      $g_comida     = (string)($form["g_comida"] ?? "");
      $g_transporte = (string)($form["g_transporte"] ?? "");
      $g_otros      = (string)($form["g_otros"] ?? "");
      $meta         = (string)($form["meta"] ?? "");
      $plazoMeses   = (string)($form["plazoMeses"] ?? "");
    }
  }

  // No calculamos ni llamamos a Ollama en autofill
  $result = null;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $action === "ask") {
  ...
  $system =
    ...
    "Adapta los tips a la situación del usuario.\n".
    ...
    <div class="content">
      <form method="post">
        ...
        <div class="actions">
          ...
          <button type="submit" name="action" value="autofill" class="btn-secondary">Autorrellenar última</button>
        </div>
      </form>
    </div>
